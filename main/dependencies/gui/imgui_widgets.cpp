...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(32, 35, 34), 4, 0, 1.f);
...
bool ImGui::TabButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool selected)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (selected)
    {
        PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(28, 188, 210)));
        RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
        PopStyleColor();
    }
    else
    {
        PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(155, 155, 155)));
        RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
        PopStyleColor();
    }
...
{
    return TabButtonEx(label, size_arg, ImGuiButtonFlags_None, selected);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImGui::PushFont(InterMedium);
    {
        ...
        window->DrawList->AddRect(ImVec2(total_bb.Min.x + 10, total_bb.Min.y + 10), ImVec2(total_bb.Min.x + 30, total_bb.Min.y + 30), ImColor(32, 35, 34), 5, 15, 1.5);


        if (*v)
            RenderCheckMark(window->DrawList, { total_bb.Max.x - 265, total_bb.Min.y + 28 / 2 }, ImColor(255, 255, 255), 10);

        if (label_size.x > 0.0f)
            window->DrawList->AddText(ImVec2(total_bb.Min.x + 40, total_bb.Min.y + 20 - ImGui::CalcTextSize(label).y / 2), ImColor(84, 86, 86), label);
    }
    ImGui::PopFont();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(0, 17));
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : CalcItemWidth() + 10;
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilledMultiColor(bb.Min, ImVec2(value_x2, bb.Max.y), ImColor(31, 31, 31), ImColor(31, 31, 31), ImColor(36, 36, 36), ImColor(36, 36, 36));
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ImU32 bg_col = GetColorU32((popup_open || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRectFilledMultiColor(ImVec2(value_x2, bb.Min.y), bb.Max, ImColor(31, 31, 31), ImColor(31, 31, 31), ImColor(36, 36, 36), ImColor(36, 36, 36));

        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.59f, 0.59f, 0.59f, 1.00f));
        ImGui::PushFont(comboarrow);
        ImGui::PopFont();
        ImGui::PopStyleColor();
    }
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(bb.Min.x + style.ItemInnerSpacing.x - 2, bb.Min.y + style.FramePadding.y - 21), label);
...
template<typename TYPE>
static const char* ImAtoi1(const char* src, TYPE* output)
{
    int negative = 0;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float w = CalcItemWidth();
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(window->Size.x, 20 + label_size.y));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ImRect grab_bb;
    ...
    if (SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb))
        MarkItemEdited(id);

    //bg
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y + 5), ImColor(23, 25, 24, int(255 * GetStyle().Alpha)), 0);
    window->DrawList->AddRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y + 5), ImColor(32, 35, 34, int(255 * GetStyle().Alpha)), 5, 15, 1.5);

    //line
    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 7, frame_bb.Min.y + 32), ImVec2(grab_bb.Max.x - 5, grab_bb.Max.y - 5), ImColor(255, 255, 255, int(255 * GetStyle().Alpha)), 0, 0);
    }
    ...
    ImGui::PushFont(InterMedium);
    {
        PushStyleColor(ImGuiCol_Text, ImColor(83, 86, 93, int(255 * GetStyle().Alpha)).operator ImVec4());
        auto label_buffer_size = CalcTextSize(value_buf, value_buf_end, true);
        RenderText(ImVec2(frame_bb.Max.x - label_buffer_size.x - 8, frame_bb.Min.y + 6), value_buf, value_buf_end);

        RenderText(ImVec2(frame_bb.Min.x + 10, frame_bb.Min.y + 6), label);
    ...
    return SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
}
...
bool ImGui::tab(const char* icon, bool selected)
{
    ...
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(28, 30, 29, int(255 * GetStyle().Alpha)), 8);
    GetWindowDrawList()->AddRect(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(32, 35, 34, int(255 * GetStyle().Alpha)), 5, 15, 1.5);

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 5, bb.Min.y + 5), ImColor(107, 107, 107, int(255 * GetStyle().Alpha)), icon);
    if (selected)
    {
        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 5, bb.Min.y + 5), ImColor(255, 255, 255, int(255 * it_filled->second)), icon);
    }
...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddRectFilled(pos, p + size_arg, ImColor(15, 17, 16, int(255 * alpha)));

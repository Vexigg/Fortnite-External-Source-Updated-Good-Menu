	...
	public:
		...
		auto ActorLoop() -> void {
			...
			if (target_entity && globals->aimbot) {
				...
				FVector rootHeadOut = Utilities->ProjectWorldToScreen(rootHead);
					...
					if (Utilities->IsVisible(closest_mesh)) {
						if (GetAsyncKeyState_Spoofed(globals->aimbot_key))
						{
							if (globals->prediction) {
								...
								float projectileSpeed = 61337; // Change it to real value, if u have more than 80iq u can make it work for all weapons :>
								float projectileGravityScale = 10.5f;  // Change it to real value, if u have more than 80iq u can make it work for all weapons :>
